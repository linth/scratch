...
main(int argc, char *argv[]) {
    ...
    uint16_t numberOfUes = 2 + numberOfUes_fix; // including numberOfUes_fix.  // 20.
    ...
    lteHelper->SetHandoverAlgorithmAttribute ("NeighbourCellOffset",
                                                   UintegerValue (1));
    /* ***************************** 
       No Optimazed Handover Algorithm.
       ***************************** */
    ...
    NodeContainer ueNodes;  // UE node.
    NodeContainer enbNodes; // eNB node.
    ...
    MobilityHelper enbMobility; // Install Mobility Model in eNB; eNB mobility helper.
    enbMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    enbMobility.SetPositionAllocator(enbPositionAlloc);
    enbMobility.Install(enbNodes);

    MobilityHelper ueMobility; // Install Mobility Model in UE; UE mobility helper.
    ueMobility.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
    ueMobility.Install(ueNodes);
    /* If you want to test a handover procedure for a mobile node, you can use the following code. */ 
    // ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (0, yForUe, 0)); // for 1 UE node.
    // ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed, 0, 0));
    for (uint16_t i = 0; i < numberOfUes; i++) // install location for several UE nodes.
    {
        // 20, 
        if (i < 21) { 
            // ueMobility.SetMobilityModel ("ns3::ConstantVelocityMobilityModel");
    ...
    // Install LTE Net Devices in eNBs and UEs
    ...
    for (uint16_t i = 0; i < numberOfUes; i++) // ueNodes
    {
        if (i == 0) {
            ...
            /* ******************************************
               attach one or more UEs to a single eNodeB.
               ****************************************** */
    ...
    for (uint32_t u = 0; u < numberOfUes; ++u) // ueNodes
    {
        ...
        for (uint32_t b = 0; b < numBearersPerUe; ++b) {
            ...
            // uplink
            // NS_LOG_LOGIC ("iL nstalling UDP Uapp for UE " << u);
    ...
    Config::Connect("/NodeList/*/$ns3::MobilityModel/CourseChange",
            MakeCallback(&CourseChange));   // location.
    ...
    monitor->CheckForLostPackets(); // calculate packet loss.
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i) 
    {
        ...
        // The minimum of PLR & the maximum of PLR.
        ...
        numberOfPacketLoss += numberOfPacketLoss;   // The number of packet loss. (Total)
        // Average throughput.
        throughput = ((i->second.rxBytes * 8.0) / ((i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds()))) / 1024;
        if (i->first != 1) {
